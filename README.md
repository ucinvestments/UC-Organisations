...
│
└── handlers/                      # Organization handlers
...
```bash
python handlers/<org_directory>/scraper.py
```
...
```bash
# UCOP organization
python handlers/ucop/ethics_compliance_audit_services/scraper.py

# Campus (when implemented)
python handlers/campuses/berkeley/scraper.py
```
...
flask scrape handlers/ucop/ethics_compliance_audit_services

# Scrape a campus (when implemented)
flask scrape handlers/campuses/berkeley
...
```bash
# UCOP organization
curl http://localhost:5000/api/organization/handlers/ucop/academic_affairs

# Campus (when implemented)
curl http://localhost:5000/api/organization/handlers/campuses/berkeley
```
...
curl -X POST http://localhost:5000/scrape/handlers/ucop/ethics_compliance_audit_services

# Scrape a campus (when implemented)
curl -X POST http://localhost:5000/scrape/handlers/campuses/berkeley
...
```bash
mkdir handlers/ucop/new_organization
```
...
```python
UCOP_ORGANIZATIONS = {
    "handlers/ucop/new_organization": {
...
class NewOrgScraper(BaseScraper):
    def __init__(self):
        org_config = ORGANIZATIONS['handlers/ucop/new_organization']
        super().__init__(
            org_name=org_config['name'],
            org_dir='handlers/ucop/new_organization',
...
- `handlers/campuses/campus_name/scraper.py`
- `handlers/labs/lab_name/scraper.py`
